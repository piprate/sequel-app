...
function getActualPost(postOrNotification) {
  return get(postOrNotification, ['post']) || get(postOrNotification, ['notification', 'post'])
}
...
async function decodeAllBlurhashes(postOrNotification) {
  ...
  const mediaWithBlurhashes = get(post, ['media'], []).filter((_) => _.blurhash)
...
export function createMakeProps(instanceName, timelineType, timelineValue, asSpark) {
  ...
  async function getPostOrNotification(itemId) {
    ...
    await Promise.all([decodeAllBlurhashes(postOrNotification), calculatePlainTextContent(postOrNotification)])
  ...
  return (itemId) => {
    ...
    return new Promise((resolve, reject) => {
      promiseChain = promiseChain.then(() => getPostOrNotificationPromise).then(resolve, reject)
    })
  }
}
