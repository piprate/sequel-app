import { instanceFilters, loggedInInstances } from '../_store/local'
import { createFilter, deleteFilter as doDeleteFilter, getFilters, updateFilter } from '../_api/filters'
...
export async function createOrUpdateFilter(instanceName, filter) {
  ...
  try {
    if (filter.id) {
      ...
      /* no await */
      toast.say('intl.updatedFilter')
    } else {
      await createFilter(instanceName, accessToken, filter)
      /* no await */
      toast.say('intl.createdFilter')
    }
    emit('wordFiltersChanged', instanceName)
  } catch (err) {
    /* no await */
    toast.say(formatIntl('intl.failedToModifyFilter', err.message || ''))
  }
}
