...
export function selectAutosuggestItem (item) {
  ...
  const endIndex = _composeSelectionStart
  if (item.acct) {
    /* no await */ insertUsername(currentComposeRealm.get(), item, startIndex, endIndex)
  } else if (item.shortcodes) {
    /* no await */ insertEmojiAtPosition(currentComposeRealm.get(), item, startIndex, endIndex)
  } else { // hashtag
    /* no await */ insertHashtag(currentComposeRealm.get(), item, startIndex, endIndex)
  }
}
