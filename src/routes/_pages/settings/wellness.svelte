...
<SettingsLayout page="settings/wellness" label={intl.wellness}>
  ...
  <form class="ui-settings">
    <label class="setting-group">
      <input bind:checked={enableAll} type="checkbox" id="choice-check-all" on:change={onCheckAllChange} />
      {intl.enableAll}
    </label>
  </form>
  ...
  <form bind:this={metricsForm} class="ui-settings">
    <label class="setting-group">
      <input
        bind:checked={hideSubscriberCount}
        name="hideSubscriberCount"
        type="checkbox"
        id="choice-disable-subscriber-counts"
      />
  ...
  <form bind:this={immediacyForm} class="ui-settings">
    <label class="setting-group">
      <input
        bind:checked={hideUnread}
        name="hideUnread"
        type="checkbox"
        id="choice-disable-unread-notification-counts"
      />
      {intl.hideUnread}
    </label>
    <label class="settings-group">
      <input
        bind:checked={showAbsoluteTimestamps}
        name="showAbsoluteTimestamps"
        type="checkbox"
        id="choice-disable-relative-timestamps"
      />
  ...
  <InfoAside className="wellness-aside">
    ...
    <a data-sveltekit-preload-data href="/settings/instances{$currentInstance ? '/' + $currentInstance : ''}"
      >{intl.filterNotificationsText}</a
    >.
