<script>
  ...
  $: instanceStates = ((list) =>
    list.map(({ name, current, access_token }, index) => ({
      index,
      current,
      name,
...
<SettingsLayout page="settings/instances" label={intl.instances}>
    ...
    <RadioGroup id="instance-switch" label={intl.switchToInstance} length={$loggedInInstancesAsList.length}>
      <SettingsList label={intl.instances}>
        {#each instanceStates as instance}
          <SettingsListRow>
            <SettingsListButton
              className="instance-switcher-instance-name"
            ...
            <div class="instance-switcher-button-wrapper">
              ...
              >
                <SvgIcon className="instance-switcher-button-svg" href={instance.current ? '#fa-star' : '#fa-star-o'} />
              </RadioGroupButton>
            </div>
          </SettingsListRow>
        {/each}
  ...
  {:else}
    ...
    <p>
      ...
      <a data-sveltekit-preload-data href="/settings/instances/register" id="log-in-link-2"
        >{intl.signUpToAnInstanceText}</a
      >
