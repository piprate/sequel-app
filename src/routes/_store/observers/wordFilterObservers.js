...
export function wordFilterObservers() {
  ...
  function updateUnexpiredInstanceFiltersIfUnchanged(now, instanceFilters) {
    const _unexpiredInstanceFilters = Object.fromEntries(
      Object.entries(instanceFilters).map(([instanceName, filters]) => {
        const unexpiredFilters = filters.filter(
          (filter) => !filter.expires_at || new Date(filter.expires_at).getTime() >= now
        )
  ...
  unexpiredInstanceFilterRegexes.subscribe(async (_unexpiredInstanceFilterRegexes) => {
    ...
    await Promise.all(
      Object.entries(_unexpiredInstanceFilterRegexes).map(async ([instanceName, contextsToRegex]) => {
        ...
        await Promise.all(
          summariesToUpdate.map(async (summary) => {
            try {
