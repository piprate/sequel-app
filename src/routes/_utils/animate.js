...
function generateCss(id, animations) {
  const keyframes = animations.map(
    ({ properties }, i) =>
      `@keyframes keyframe-${id}-${i} {
      ${properties
        .map(
          (obj, i) => `${Math.round((100 * i) / (properties.length - 1))}% {
          ${Object.keys(obj)
            .map((key) => `${key}: ${obj[key]};`)
            .join('')}
        }`
...
export function animate(el, animations) {
  if (typeof el.animate === 'function') {
    return animations.map(({ properties, options }) => el.animate(properties, options)).map((anim) => anim.play())
  }
  ...
  requestAnimationFrame(() => {
    el.classList.add(id)
    const wait = Math.max.apply(
      Math,
      animations.map(({ options }) => options.duration)
    )
    setTimeout(() => {
      requestAnimationFrame(() => {
