    ...
    <FreeTextLayout>
      ...
      {:else}
        {#if sparks && sparks.length}
                ...
                <SearchResult>
                  <div class="search-result-spark" on:click={(event) => onSwitchToThisSpark(event, spark)}>
                    ...
                    <div class="search-result-spark-stats">
                      Posts: {spark.data.postCount || 0} Bubbles: {spark.data.bubbleCount || 0} Subscribers: {spark.data
                        .subscriberCount || 0}
                    </div>
                    <div class="spark-selector-button-wrapper">
                      ...
                      >
                        <SvgIcon
                          className="spark-selector-button-svg"
          ...
          <a class="button primary new-spark-button" data-sveltekit-preload-data href="/sparks/new"
            >{intl.createNewSpark}</a
          >
        {:else if loadError}
          <ErrorMessage error={loadError} />
        {:else}
          <a class="button primary new-spark-button" data-sveltekit-preload-data href="/sparks/new"
            >{intl.createFirstSpark}</a
          >
        {/if}
