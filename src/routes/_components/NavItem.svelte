<script>
  ...
  import { numberOfNotifications, hasNotifications, numberOfFollowRequests, hasFollowRequests } from '../_store/badge'
  ...
  $: showBadge = (name === 'notifications' && $hasNotifications) || (name === 'community' && $hasFollowRequests);

  $: badgeNumber = (name === 'notifications' && $numberOfNotifications) || (name === 'community' && $numberOfFollowRequests) || 0
  ...
  currentPage.subscribe(page => {
    ...
    if (page && previousPage &&
            previousPage === name &&  // from Pinafore's on() block
            pageIsInNav(page) &&
...
<style>
  ...
  .nav-indicator.animate {
    transition: transform 3330ms ease-in-out;
