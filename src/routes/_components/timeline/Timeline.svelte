<script>
  ...
  $: makeProps = createMakeProps($currentInstance, timelineType, timelineValue);
  $: label = (() => {
  ...
  const handleItemIdsToAdd = () => {
    ...
    if (timelineType === 'status') {
      ...
      setForTimeline(shouldShowHeader, $currentInstance, timeline, true);
      // unless the user has disabled infinite scroll entirely
      if ($disableInfiniteScroll) {
        setForTimeline(showHeader, $currentInstance, timeline, true);
      }
    }
  ...
  function onScrollToTop() {
    if ($shouldShowHeader) {
      setForTimeline(showHeader, $currentInstance, $currentTimeline, true);
      setForTimeline(shouldShowHeader, $currentInstance, $currentTimeline, false);
    }
  }

  function onScrollTopChanged (_scrollTop) {
