<script>
  ...
  $: membershipModeOptions = markSelectedOption(
    BUBBLE_MEMBERSHIP_MODE_OPTIONS,
    template.membershipMode || 'invite_only'
  )
  ...
  <div class="bubble-form-box">
    <form on:submit={onSubmitBubble} aria-label={formLabel}>
      ...
      <input
        ...
        placeholder={intl.enterBubbleName}
        required
      />
      ...
      <InfoAside className="notice-aside">
        {selectedMembershipMode.description}
        {selectedObserverMode.description}
        {selectedWriterMode.description}
        {selectedFederationMode.description}
        {'intl.useButtonsAbove'}
      </InfoAside>
      ...
      <MediaField
        ...
        field="avatar"
        elementId="avatar"
        buttonLabel={intl.addAvatarImage}
        enableNFT={true}
        mediaProfile="avatar"
      />
      <InfoAside className="notice-aside">
