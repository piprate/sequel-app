...
<ModalDialog {id} {label} {title} background="var(--main-bg)">
  ...
  {#if sparks && sparks.length}
    <RadioGroup
      id="spark-selector"
      className="spark-selection-radio"
      label={intl.selectSparkFromList}
                ...
                <div class="search-sparks-spark-summary">
                  {spark.data.summary
                    ? spark.data.summary.length > 250
                      ? spark.data.summary.substring(0, 250) + '...'
                      : spark.data.summary
                    : ''}
                </div>
                <div class="spark-selector-button-wrapper">
                  <SvgIcon className="spark-selector-button-svg {spark.selected ? '' : 'hidden'}" href="#fa-check" />
                </div>
              </div>
            </RadioGroupButton>
          </li>
  ...
  {:else}
    <div class="no-results">No Sparks Found</div>
  {/if}
</ModalDialog>
