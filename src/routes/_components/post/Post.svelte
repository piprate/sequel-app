<script>
  ...
  $: isPostInOwnThread = (
          (timelineType === 'post' || timelineType === 'reply') && timelineValue === postUnwrappedId
  )
  $: isPostInNotification = (
          notification && notification.subjectPost &&
          notification.type !== 'mention' && notification.subjectPost.id === postId
  )
...
>
  ...
  {#if showPoll && (showContent || preloadHiddenContent)}
    <PostPoll {notification} {uuid} {isPostInNotification} {isPostInOwnThread} {post} {postEmojis} shown={showContent} />
  {/if}
  {#if showLocation}
