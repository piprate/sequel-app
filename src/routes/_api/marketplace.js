...
export async function initialiseMintOnDemand(
  instanceName,
  accessToken,
  id,
  buyerAddr,
...
export async function getMintOnDemandSignature(
  instanceName,
  accessToken,
  id,
  buyerAddr,
...
export async function withdrawMarketplaceListing(instanceName, accessToken, id, asSpark, limit = 50) {
  ...
  return populateMediaURLsInSingleListing(
    post(url, null, sequelAuth(accessToken, asSpark), { timeout: DEFAULT_TIMEOUT }),
    instanceName
  )
}
